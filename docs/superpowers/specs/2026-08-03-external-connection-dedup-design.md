...
- 有失敗：`已同步 N 個外部連線，M 個失敗`
- 兩者皆有：`已同步 N 個外部連線（M 個與現有連線重複已略過），K 個失敗`

重複數與失敗數是兩件獨立的事，訊息各自附加。前綴統一為「已同步 N 個外部連線」。
...
**伺服器欄位的等價寫法判不出來。** 外部來源用 IP、使用者手動那筆用主機名稱或 FQDN 指向同一台機器時，判為不重複，兩筆都會顯示。同族的還有 `sql01` 與 `sql01,1433`、`tcp:sql01`、`sql01\INSTANCE`。要正確處理需要 DNS 解析與連線字串正規化，成本與誤判風險都不划算，接受此限制。

**不比密碼可能留下連不上的那筆。** 當內部連線存的是舊密碼、外部那筆是 vault 解出的新密碼時，被隱藏的正是能連的那筆。仍然不比密碼：反過來（納入比對）會讓去重在密碼有落差時全面失效，而密碼過期本來就該回頭修內部連線。

**停用的內部連線會讓該資料庫在此視窗無法連線。** `CanConnect` 要求 `SelectedProfile.IsEnabled`，而重複的外部連線已被隱藏，兩者疊加使該資料庫沒有可用入口。畫面上唯一線索是狀態訊息的略過筆數，不會指出是哪一筆、也不會說它已停用。解法是把內部那筆重新啟用。

**同步後異動內部連線不會重新過濾。** `Save()`／`Delete()` 只重載 `Profiles`，`ExternalProfiles` 維持同步當下的快照。影響方向是安全的一邊（多顯示而非少顯示），重按同步即可修正，不為此保存原始同步結果。
