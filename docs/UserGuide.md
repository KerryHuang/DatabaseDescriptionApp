...
在左側清單選取要調整的連線，右側編輯表單的「啟用此連線」勾選框用來啟用／停用它，按「儲存」後生效。

停用的連線在左側清單以灰階呈現，方便辨識。
...
整合三項資料庫完整性資訊，協助 DBA 快速判斷健康狀態：

| 區塊 | 內容 | 資料來源 |
|------|------|---------|
| 各資料庫最後 CHECKDB 時間 | 顯示每個 DB 最後一次成功 CHECKDB 的時間、距今天數、健康分級 | `DBCC DBINFO` 的 `dbi_dbccLastKnownGood` |
| Suspect Pages | 列出 SQL Server 偵測到的疑似損毀頁面（正常為空，顯示綠色「✅ 目前無疑似損毀頁面」） | `msdb.dbo.suspect_pages` |
| 最近 CHECKDB Job 執行紀錄 | 列出名稱含 `CheckDb` 的 SQL Agent Job 最近 50 筆執行歷史（時間、時長、結果、訊息） | `msdb.dbo.sysjobhistory` JOIN `sysjobs` |
...
| Healthy | 距今 < 14 天 |
| Warning | 距今 14–29 天 |
| Critical | 距今 ≥ 30 天 或 從未檢查 |
| Unknown | 查詢失敗（權限不足或 DB 離線） |

> 💡 搭配維護計劃的「建立每週完整性檢查排程」自動產生 `_CheckDb` Job 後，每週執行紀錄會出現在這裡。
...
| 項目 | 說明 | 檢查內容 |
|------|------|----------|
| 更新相容性層級至當前版本 | 將 DB 相容性層級提升到當前 SQL Server 版本 | 比對 DB 與伺服器層級 |
| 設定 Recovery Model 為 SIMPLE | 設定主庫和測試庫 | 查詢目前模式 |
| 調整檔案自動成長設定 | 將 mdf/ldf 的 autogrowth 改為固定 MB（預設 256/128 MB）以避免線上抖動 | 檢查所有檔案是否為固定且 ≥ 64 MB |
| 預擴資料檔（保留成長緩衝） | 將資料檔擴充至「目前大小 + 緩衝（預設 5 GB）」並湊整 GB；含磁碟空間護欄 | 計算可用率，可用 < 20% 建議預擴；磁碟不足自動跳過 |
...
| 建立每週完整性檢查排程 | 建立每週執行 `DBCC CHECKDB ... PHYSICAL_ONLY` 的 SQL Agent Job（預設週日 03:00） | 檢查 `{db}_CheckDb` Job 是否存在 |
