...
- **Schema 差異分析** - 比對基準與目標資料庫的結構差異 (Ctrl+Shift+M)
- **風險等級標示** - 每項差異標記低/中/高風險，禁止自動執行的項目明確標示
- **分析建議報告** - 分析完成後自動產生完整文字報告，含執行建議順序和風險清單
- **多維度篩選** - 支援風險等級、物件類型、差異類型多選篩選，差異類型根據實際資料動態產生
- **勾選執行** - 手動勾選要執行的差異項目，預設按風險排序
- **預覽 SQL** - 彈出視窗顯示完整 T-SQL 腳本，支援一鍵複製
- **下載 SQL** - 將腳本儲存為 `.sql` 檔案
- **Dry Run（模擬執行）** - 語法驗證，自動回滾，不實際變更目標資料庫
- **執行 Migration** - 對目標資料庫執行選取的 Migration，含自動回滾保護
- **執行報告** - 每次執行後顯示詳細報告（狀態、耗時、備註），可下載
...
### 16. Schema Migration（資料庫遷移）
1. 按 Ctrl+Shift+M 或選單「工具 > Schema Migration」
2. 選擇基準資料庫（來源）和目標資料庫（需要更新的環境）
3. 點擊「分析差異」，系統分析兩個資料庫的 Schema 差異並產生建議報告
4. 使用篩選器（風險、物件類型、差異類型）縮小要處理的範圍
5. 勾選要執行的差異項目（高/禁止風險項目預設不可勾選）
6. 可選擇：
   - **預覽 SQL** - 查看將執行的 T-SQL 腳本
   - **下載 SQL** - 儲存腳本為 `.sql` 檔案後手動執行
   - **Dry Run** - 模擬執行（不實際變更），驗證腳本語法正確
   - **執行 Migration** - 正式套用到目標資料庫（含自動回滾保護）
7. 執行完成後可查看執行報告並下載
