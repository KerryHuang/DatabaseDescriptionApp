...
public class MaintenancePlanSqlGenerator : IMaintenancePlanSqlGenerator
{
    ...
    private static string GenerateRenameLogicalFiles(MaintenancePlanConfig config)
    {
        ...
        sb.AppendLine($"    DECLARE @oldName SYSNAME, @newName SYSNAME, @renameSql NVARCHAR(MAX);");
        sb.AppendLine();
        sb.AppendLine($"    -- 3.1 動態查詢目前的邏輯資料檔，若名稱與 '{dbName}_Data' 不同則更名");
        sb.AppendLine($"    PRINT N'3.1 檢查並重新命名邏輯資料檔 (Data)...';");
        sb.AppendLine($"    SET @newName = @dbName + N'_Data';");
        sb.AppendLine($"    SET @oldName = NULL;");
        sb.AppendLine($"    SELECT TOP (1) @oldName = name");
        sb.AppendLine($"    FROM sys.master_files");
        sb.AppendLine($"    WHERE database_id = DB_ID(@dbName)");
        sb.AppendLine($"      AND type_desc = 'ROWS'");
        sb.AppendLine($"      AND name <> @newName");
        sb.AppendLine($"    ORDER BY file_id;");
        sb.AppendLine($"    IF @oldName IS NOT NULL");
        sb.AppendLine($"    BEGIN");
        sb.AppendLine($"        PRINT N'3.1-1 重新命名邏輯 Data 檔: ''' + @oldName + N''' → ''' + @newName + N'''';");
        sb.AppendLine($"        SET @renameSql = N'ALTER DATABASE ' + QUOTENAME(@dbName) +");
        sb.AppendLine($"            N' MODIFY FILE (NAME = N''' + REPLACE(@oldName, '''', '''''') + N''', NEWNAME = N''' + REPLACE(@newName, '''', '''''') + N''');';");
        sb.AppendLine($"        EXEC sp_executesql @renameSql;");
        sb.AppendLine($"        PRINT N'3.1-2 完成 Data 檔重新命名';");
        sb.AppendLine($"    END");
        sb.AppendLine($"    ELSE");
        sb.AppendLine($"    BEGIN");
        sb.AppendLine($"        PRINT N'3.1-3 資料檔已是正確名稱或不存在需更名的檔案';");
        sb.AppendLine($"    END");
        sb.AppendLine();
        sb.AppendLine($"    -- 3.2 動態查詢目前的邏輯日誌檔，若名稱與 '{dbName}_Log' 不同則更名");
        sb.AppendLine($"    PRINT N'3.2 檢查並重新命名邏輯日誌檔 (Log)...';");
        sb.AppendLine($"    SET @newName = @dbName + N'_Log';");
        sb.AppendLine($"    SET @oldName = NULL;");
        sb.AppendLine($"    SELECT TOP (1) @oldName = name");
        sb.AppendLine($"    FROM sys.master_files");
        sb.AppendLine($"    WHERE database_id = DB_ID(@dbName)");
        sb.AppendLine($"      AND type_desc = 'LOG'");
        sb.AppendLine($"      AND name <> @newName");
        sb.AppendLine($"    ORDER BY file_id;");
        sb.AppendLine($"    IF @oldName IS NOT NULL");
        sb.AppendLine($"    BEGIN");
        sb.AppendLine($"        PRINT N'3.2-1 重新命名邏輯 Log 檔: ''' + @oldName + N''' → ''' + @newName + N'''';");
        sb.AppendLine($"        SET @renameSql = N'ALTER DATABASE ' + QUOTENAME(@dbName) +");
        sb.AppendLine($"            N' MODIFY FILE (NAME = N''' + REPLACE(@oldName, '''', '''''') + N''', NEWNAME = N''' + REPLACE(@newName, '''', '''''') + N''');';");
        sb.AppendLine($"        EXEC sp_executesql @renameSql;");
        ...
        sb.AppendLine($"    ELSE");
        sb.AppendLine($"    BEGIN");
        sb.AppendLine($"        PRINT N'3.2-3 日誌檔已是正確名稱或不存在需更名的檔案';");
        sb.AppendLine($"    END");
    ...
    public string GenerateExportSql(MaintenancePlanConfig config, IReadOnlyList<StepCheckResult> checkResults)
    {
        ...
        if (renameStep is not null)
        {
            ...
            sb.AppendLine("-- 說明: 動態查詢目前的邏輯檔名 (Data/Log)，若與資料庫名稱不一致則更名為 {db}_Data / {db}_Log");
            sb.AppendLine("-- ============================================================");
            ...
            sb.AppendLine("    USE [master];");
            sb.AppendLine();
            sb.AppendLine("    -- 重新命名邏輯資料檔 (動態查詢實際名稱)");
            sb.AppendLine("    SET @newFileName = @DatabaseName + N'_Data';");
            sb.AppendLine("    SET @oldFileName = NULL;");
            sb.AppendLine("    SELECT TOP (1) @oldFileName = name");
            sb.AppendLine("    FROM sys.master_files");
            sb.AppendLine("    WHERE database_id = DB_ID(@DatabaseName)");
            sb.AppendLine("      AND type_desc = 'ROWS'");
            sb.AppendLine("      AND name <> @newFileName");
            sb.AppendLine("    ORDER BY file_id;");
            sb.AppendLine("    IF @oldFileName IS NOT NULL");
            sb.AppendLine("    BEGIN");
            sb.AppendLine("        SET @renameSql = N'ALTER DATABASE ' + QUOTENAME(@DatabaseName) +");
            sb.AppendLine("            N' MODIFY FILE (NAME = N''' + REPLACE(@oldFileName, '''', '''''') + N''', NEWNAME = N''' + REPLACE(@newFileName, '''', '''''') + N''');';");
            sb.AppendLine("        PRINT N'  重新命名邏輯 Data 檔: ' + @oldFileName + N' → ' + @newFileName;");
            sb.AppendLine("        EXEC sp_executesql @renameSql;");
            sb.AppendLine("    END");
            sb.AppendLine("    ELSE");
            sb.AppendLine("        PRINT N'  資料檔已是正確名稱或不存在需更名的檔案';");
            sb.AppendLine();
            sb.AppendLine("    -- 重新命名邏輯日誌檔 (動態查詢實際名稱)");
            sb.AppendLine("    SET @newFileName = @DatabaseName + N'_Log';");
            sb.AppendLine("    SET @oldFileName = NULL;");
            sb.AppendLine("    SELECT TOP (1) @oldFileName = name");
            sb.AppendLine("    FROM sys.master_files");
            sb.AppendLine("    WHERE database_id = DB_ID(@DatabaseName)");
            ...
            sb.AppendLine("        SET @renameSql = N'ALTER DATABASE ' + QUOTENAME(@DatabaseName) +");
            sb.AppendLine("            N' MODIFY FILE (NAME = N''' + REPLACE(@oldFileName, '''', '''''') + N''', NEWNAME = N''' + REPLACE(@newFileName, '''', '''''') + N''');';");
            sb.AppendLine("        PRINT N'  重新命名邏輯 Log 檔: ' + @oldFileName + N' → ' + @newFileName;");
            sb.AppendLine("        EXEC sp_executesql @renameSql;");
            sb.AppendLine("    END");
            sb.AppendLine("    ELSE");
